...
```{r}
library(car)
data$Autonomia <- recode(data$Autonomia, "0=1 ; 0.25=0.75 ; 0.5=0.5 ; 0.75=0.25 ; 1=0")
...
data$Autonomia=factor(data$Autonomia,
                            ...
                            labels = c("5","4","3","2","1"),
                            ordered = T)
data$Autonomia <-as.numeric(data$Autonomia)
...
data$Violencia <- recode(data$Violencia,"0.25=0.75 ; 0.5=0.5 ; 0.75=0.25")
data$Violencia <-as.factor(data$Violencia)
data$Violencia=factor(data$Violencia,
                            levels = levels(data$Violencia),
                            labels = c("3","2","1"),
                            ordered = T)
data$Violencia <-as.numeric(data$Violencia)
table(data$Violencia)
```

###Normalizamos la variable Desempleo
```{r}
library(BBmisc)
data$Desempleo= normalize(data$Desempleo, 
                       method = "range", 
                       margin=2, # by column
                       range = c(0, 100),)
data$Desempleo=round(data$Desempleo,2)
```
...
```{r}
hipotesis5 = formula(DesigualdadGenero ~ Autonomia + VozPolitica + Desempleo + CuentaF)
#como son todas las demas las v. independientes se pone "."
...
```{r}
hipotesis6 = formula(DesigualdadGenero ~ Autonomia+ VozPolitica  + LibertadMov + Desempleo + CuentaF)
#como son todas las demas las v. independientes se pone "."
...
```{r}
hipotesis7 = formula(DesigualdadGenero ~ Autonomia+ VozPolitica  + LibertadMov + Desempleo + AccesoJ + CuentaF)
#como son todas las demas las v. independientes se pone "."
...
```{r}
hipotesis8 = formula(DesigualdadGenero ~ Autonomia+ VozPolitica + CuentaF)
#como son todas las demas las v. independientes se pone "."
...
```
```{r}
hipotesis9 = formula(DesigualdadGenero ~ Autonomia  + LibertadMov + CuentaF)
#como son todas las demas las v. independientes se pone "."
...
```{r}
hipotesis10 = formula(DesigualdadGenero ~ Autonomia + CuentaF)
#como son todas las demas las v. independientes se pone "."
