...
data = data[,-1] #puede ser por data[,1]=NULL o data$...=NULL
data = na.omit(data) #omitir peridods
```

2. Calculamos las distanias
```{r}
head(data)[, c(3:5)]#con que columnas hago el conglomerado
dist = daisy(data[,c(3:5)], metric = "gower") #gower permite categoricas, para...
```

3. Cluster particionante
```{r}
# Calculamos los cluster
regiones_part = pam(dist, 3, cluster.only = T) #cluster.only para que solo salga los cluster
...
```{r}
# Calculamos los cluster
regiones_aglo <- hcut(dist, k = 3 , hc_func='agnes',hc_method = "ward.D") #library facoextra, poner el K sino sera 2 por default

# Creamos una variable en la data con el cluster creado
data$Caglomerativo = regiones_aglo$cluster
# Visualizamos los cluster
...
5. Cluster divisivo
```{r}
regiones_div <- hcut(dist, k = 3 , hc_func='diana')#no se usa nignu metodo
