...
data$Autonomia,  <- recode(data$Autonomia, "0=0 ; 0.25=0 ; 0.5=0.5 ; 0.75=1 ; 1=1")
...
levels(data$Autonomia )<-c("Bueno","Regular","Malo")
#verificamos
str(data$Autonomia)
table(data$Autonomia )
```
...
#Le asignamos etiquetas a los valores
levels(data$Violencia)<-c("Bueno","Regular","Malo")
#verificamos
str(data$Violencia)
table(data$Violencia)
```
...
hist(data$Violencia, main = "Desigualdad de genero")
```

PASO 2;:VARIABLE NUMERICA Autonomia reproductiva
...
describe.by(data$Autonomia)
```

-Analisis grafico de variable
```{r}
#Solicitamos histograma
barplot(table(data$DesigualdadGenero),col="red",
        xlab=NULL,main="Autonomia Reproductiva")
```

PASO 3: VARIABLE NUMERICA Violencia contra la mujer
```{r}
#Solicitamos estadisticos descriptivos
...
```{r}
#o tambien se puede utilizar:
describe.by(data$Violencia)
```

-Analisis grafico de variable
```{r}
#Solicitamos una grafica de barras
barplot(table(data$Autonomia),col="lightgreen",
...
```{r}
#Gráfico de medias con intervalos de confianza (ver superposición)
plotmeans(data$DesigualdadGenero ~ Violencia, data = data)
```
